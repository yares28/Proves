...
/**
 * Parses ISO 8601 duration format (e.g., "P1D", "P3D") to number of days
 * @param durationStr - ISO 8601 duration string like "P1D", "P3D", "P5D"
 * @returns number of days or null if invalid
 */
function parseDurationDay(durationStr: string | null | undefined): number | null {
  if (!durationStr || typeof durationStr !== 'string') {
    return null;
  }

  // Match ISO 8601 duration format: P[n]D (where n is the number of days)
...
/**
 * Determines if an exam should be treated as a whole day event
 * @param exam - The exam object
 * @returns true if it's a whole day event, false otherwise
 */
function isWholeDayEvent(exam: Exam): boolean {
  return Boolean(exam.duration_day && parseDurationDay(exam.duration_day) !== null);
}
...
export function generateICalContent(
  ...
  exams.forEach((exam) => {
    const isWholeDay = isWholeDayEvent(exam);
    ...
    // Validate duration logic: exam must have either duration_day OR (time + duration_minutes)
    if (isWholeDay) {
      // For whole day events, duration_day must be valid
      const durationDays = parseDurationDay(exam.duration_day);
      if (!durationDays || durationDays <= 0) {
        invalidExams.push({ exam, reason: "Invalid duration_day format" });
        return;
      }
    } else {
      // For timed events, must have valid time and duration_minutes
      if (!hasTime) {
        invalidExams.push({ exam, reason: "Missing exam time for timed event" });
        return;
      }
      ...
      if (!exam.duration_minutes || exam.duration_minutes <= 0) {
        invalidExams.push({ exam, reason: "Missing or invalid duration_minutes" });
        return;
      }
    }
  ...
  validExams.forEach((exam) => {
    ...
    if (isWholeDay) {
      ...
      durationDays = parseDurationDay(exam.duration_day);
    } else if (hasTime) {
      // Handle timed events
      const parseResult = parseExamDateTime(exam.date, exam.time, timeZone);
    ...
    if (isWholeDay && durationDays) {
      // --- Handle whole day events with duration_day ---
      const startDate = exam.date.replace(/-/g, ""); // e.g. 20251031
      const endObj = new Date(exam.date);
      endObj.setDate(endObj.getDate() + durationDays); // Add the number of days from duration_day
      const endDate = endObj.toISOString().slice(0, 10).replace(/-/g, "");
      
      icalLines.push(
        "BEGIN:VEVENT",
        `UID:exam-${exam.id}-${exam.date}-${exam.duration_day}@upv-exam-calendar.com`,
        `DTSTART;VALUE=DATE:${startDate}`,
        `DTEND;VALUE=DATE:${endDate}`,
        foldLine(
          `SUMMARY:${escapeICalText(
            exam.subject + " - Exam" + (durationDays > 1 ? ` (${durationDays} días)` : "")
          )}`
        ),
      ...
      // --- Handle timed exams with exam_time and duration_minutes ---
      icalLines.push(
        "BEGIN:VEVENT",
      ...
      // --- Fallback for exams with missing time info (legacy support) ---
      const startDate = exam.date.replace(/-/g, ""); // e.g. 20251031
      ...
      icalLines.push(
        "BEGIN:VEVENT",
        `UID:exam-${exam.id}-${exam.date}-unknown@upv-exam-calendar.com`,
        `DTSTART;VALUE=DATE:${startDate}`,
        `DTEND;VALUE=DATE:${endDate}`,
        foldLine(
          `SUMMARY:${escapeICalText(
...
function generateUPVCompatibleICalContent(
  ...
  exams.forEach((exam) => {
    ...
    // Validate duration logic for UPV format
    if (isWholeDay) {
      const durationDays = parseDurationDay(exam.duration_day);
      if (!durationDays || durationDays <= 0) {
        return;
      ...
      // Skip exams without proper time/duration data
      return;
    }
  ...
  validExams.forEach((exam) => {
    ...
    if (isWholeDay && durationDays) {
      // --- Handle whole day events with duration_day (UPV format) ---
      const startDate = exam.date.replace(/-/g, ""); // e.g. 20251031
      const endObj = new Date(exam.date);
      endObj.setDate(endObj.getDate() + durationDays); // Add the number of days from duration_day
      const endDate = endObj.toISOString().slice(0, 10).replace(/-/g, "");
      ...
      icalLines.push(
        ...
        "SEQUENCE:0",
        "STATUS:CONFIRMED",
        foldLine(`SUMMARY:Examen ${exam.subject}` + (durationDays > 1 ? ` (${durationDays} días)` : "")),
        "TRANSP:OPAQUE",
      ...
      // --- Handle timed exams with exam_time and duration_minutes (UPV format) ---
      icalLines.push(
        "BEGIN:VEVENT",
...
export async function generateAppleCalendarSubscriptionUrlWithFilters(
  filters: Record<string, string[]>,
  calendarName: string = "UPV Exams"
): Promise<string> {
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Starting with filters:", filters);
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Calendar name:", calendarName);

  // Build the subscription URL with filters - similar to Google Calendar but for Apple
  const params = new URLSearchParams();
  params.set("name", calendarName);
  ...
  const queryString = params.toString();
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Generated query string:", queryString);
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Total filter categories:", Object.keys(filters).length);
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Filter details:", {
    schools: filters.school?.length || 0,
    degrees: filters.degree?.length || 0,
    years: filters.year?.length || 0,
    semesters: filters.semester?.length || 0,
  ...
  // Convert to webcal protocol for Apple Calendar subscription
  // Apple Calendar automatically handles webcal:// protocol and converts it to https://
  const webcalUrl = directUrl.replace(/^https?:/, "webcal:");
  console.log("🍎 [generateAppleCalendarSubscriptionUrlWithFilters] Webcal URL:", webcalUrl);
  
  return webcalUrl;
}

export function generateAppleCalendarEventUrl(exam: Exam): string {
  // Apple Calendar doesn't have a direct URL scheme like Google Calendar for individual events
  // Instead, we generate a data URL that can be used to create an event
  // This follows Apple's recommended approach for calendar integration
  ...
  // Format dates for Apple Calendar (ISO format)
  const formatAppleDate = (date: Date) => {
    return date.toISOString();
  };

  // Create event data in a format that Apple Calendar can understand
  const eventData = {
    title: `${exam.subject} - Exam`,
    startDate: formatAppleDate(startTime),
    endDate: formatAppleDate(endTime),
  ...
  // For Apple Calendar, we return a data URL that can be used with the calendar app
  // Apple Calendar supports data URLs for event creation
  const dataUrl = `data:text/calendar;charset=utf-8,${encodeURIComponent(
    generateICalContent([exam], { 
      calendarName: "UPV Exam", 
      useUPVFormat: true 
    })
  )}`;
...
export function getAppleCalendarInstructions(): string {
  // Return instructions for users on how to subscribe to the calendar in Apple Calendar
  return `To subscribe to this calendar in Apple Calendar:
...
5. Choose your preferred settings and click OK

The calendar will automatically sync and update with new exam schedules.`;
}
